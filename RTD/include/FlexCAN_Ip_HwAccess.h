 ...
 *
 *  @brief FlexCAN Ip Hardware Access Header File
 *  @details FlexCAN Ip Hardware Access Header File contains the APIs declaration
 *
 *  @addtogroup FlexCAN
 *  @{
 */
...
#define FLEXCAN_IP_HWACCESS_AR_RELEASE_MINOR_VERSION_H       9
#define FLEXCAN_IP_HWACCESS_AR_RELEASE_REVISION_VERSION_H    0
#define FLEXCAN_IP_HWACCESS_SW_MAJOR_VERSION_H               7
#define FLEXCAN_IP_HWACCESS_SW_MINOR_VERSION_H               0
#define FLEXCAN_IP_HWACCESS_SW_PATCH_VERSION_H               1
/*==================================================================================================
*                                     FILE VERSION CHECKS
==================================================================================================*/
...
/*! @brief FlexCAN Enhacend Fifo Maximum Standard Filter Elements */
#define FLEXCAN_IP_ENHANCED_RXFIFO_STD_FILTER_MAX            (128U)
/*! @brief FlexCAN Enhacend Fifo Maximum Extended Filter Elements */
#define FLEXCAN_IP_ENHANCED_RXFIFO_EXT_FILTER_MAX            (64U)
...
typedef struct
{
    uint32 code;                        /*!< MB code for TX or RX buffers.*/
                                        /*!< Defined by FlexCAN_Ip_RxMbCodeType and FlexCAN_Ip_TxMbCodeType enums */
    Flexcan_Ip_MsgBuffIdType msgIdType; /*!< Type of message ID (standard or extended)*/
    uint8 dataLen;                      /*!< Length of Data in Bytes*/
    boolean fd_enable;                  /*!< Enable/Disable FD feature*/
    uint8 fd_padding;                   /*!< Length of Padding Data in Bytes*/
 ...
 *
 */
void FlexCAN_LockRxMsgBuff(FLEXCAN_Type * pBase,
                           uint32 MsgBuffIdx
                          );
...
{
#if (FLEXCAN_IP_DEV_ERROR_DETECT == STD_ON)
    DevAssert((boolean)(NumOfWords > 0U));
#endif
...
static inline uint8 FlexCAN_GetMsgBuffIntStatusFlag(const FLEXCAN_Type * pBase, uint32 MsgBuffIdx)
{
    ...
    return (uint8)(Flag & (uint32)UINT8_MAX);
}
...
static inline uint8 FlexCAN_GetMsgBuffIntStatusImask(const FLEXCAN_Type * pBase, uint32 MsgBuffIdx)
{
    ...
    return (uint8)(u32Imask & (uint32)UINT8_MAX);
}
...
{
    uint64 NominalTimeSeg1 = ((uint64)TimeSeg->phaseSeg1 + (uint64)TimeSeg->propSeg + (uint64)1U);
#if (FLEXCAN_IP_DEV_ERROR_DETECT == STD_ON)
    DevAssert((boolean)(NominalTimeSeg1 < (uint64)FLEXCAN_ENCBT_NTSEG1_MASK));
#endif

    (pBase->ENCBT) = ((pBase->ENCBT) & ~(FLEXCAN_ENCBT_NTSEG1_MASK | FLEXCAN_ENCBT_NTSEG2_MASK | FLEXCAN_ENCBT_NRJW_MASK));

    (pBase->ENCBT) = ((pBase->ENCBT) | (FLEXCAN_ENCBT_NTSEG1(NominalTimeSeg1)    |
...
{
    uint64 DataTimeSeg1 = ((uint64)TimeSeg->phaseSeg1 + (uint64)TimeSeg->propSeg);
#if (FLEXCAN_IP_DEV_ERROR_DETECT == STD_ON)
    DevAssert((boolean)(DataTimeSeg1 < (uint64)FLEXCAN_EDCBT_DTSEG1_MASK));
#endif

    (pBase->EDCBT) = ((pBase->EDCBT) & ~(FLEXCAN_EDCBT_DTSEG1_MASK |
                                         FLEXCAN_EDCBT_DTSEG2_MASK |
                                         FLEXCAN_EDCBT_DRJW_MASK
                                        )
                     );

    (pBase->EDCBT) = ((pBase->EDCBT) | (FLEXCAN_EDCBT_DTSEG1(DataTimeSeg1) |
                                                ...
                                                )
{
    ...
    #if FLEXCAN_IP_FEATURE_MAX_MB_NUM > 32U
        if ((SubEndMbIdx >= 32U) && (SubEndMbIdx < 64U))
        {
            SubStartMbIdx = 32U;
    ...
    #if FLEXCAN_IP_FEATURE_MAX_MB_NUM > 64U
        if ((SubEndMbIdx >= 64U) && (SubEndMbIdx < 96U))
        {
            SubStartMbIdx = 64U;
 ...
 */
static inline uint8 FlexCAN_GetEnhancedRxFIFOIntStatusFlag(const FLEXCAN_Type * pBase, uint8 IntFlag)
{
    return (uint8)(((pBase->ERFSR & ((uint32)1U << (IntFlag & (uint8)0x1F))) >> (IntFlag & (uint8)0x1F)) & (uint32)UINT8_MAX) ;
}

/*!
 * @brief Clears the individual FlexCAN Enhanced Rx FIFO interrupt flag.
 *
 * @param   pBase     The FlexCAN base address
 * @param   IntFlag   Index of the Enhanced Rx FIFO interrupt flag
 */
static inline void FlexCAN_ClearEnhancedRxFifoIntStatusFlag(FLEXCAN_Type * pBase, uint8 IntFlag)
{
    (pBase->ERFSR) = ((uint32)1U << (IntFlag & (uint8)0x1F));
}

/*!
 * @brief Gets the individual FlexCAN Enhanced Rx FIFO interrupt mask.
 *
 * @param   pBase       The FlexCAN base address
 * @param   IntFlag     Index of the Enhanced Rx FIFO interrupt mask
 * @return  The individual Enhanced Rx FIFO interrupt mask (0 and 1 are the mask value)
 */
static inline uint8 FlexCAN_GetEnhancedRxFIFOIntStatusMask(const FLEXCAN_Type * pBase, uint8 IntFlag)
{
    return (uint8)(((pBase->ERFIER & ((uint32)1U << (IntFlag & (uint8)0x1F))) >> (IntFlag & (uint8)0x1F)) & (uint32)UINT8_MAX);
}
/*!
...
/*!
 * @brief Clear Memory Error Injection.
 *
 * @param   pBase          The FlexCAN base address
 * @param   ErrorMaskType  The Memory Error Injection Mask
 */
static inline void FlexCAN_ClearMemErrorInjection(FLEXCAN_Type * pBase, Flexcan_Ip_MemErrInjectionMaskType ErrorMaskType)
{
    /* check if the error is presently injected */
    if ((pBase->MECR & (uint32)ErrorMaskType) != 0U)
    {
        /* disable error injection */
        pBase->MECR &= ~(uint32)ErrorMaskType;
        /* clear error injection info */
        pBase->ERRIAR  = 0U;
        pBase->ERRIDPR = 0U;
        pBase->ERRIPPR = 0U;
    }
