 ...
 *  @file FlexCAN_Ip.c
 *
 *  @brief FlexCAN Ip Driver Source File
 *  @details Implementing functionality of FlexCAN Ip Driver APIs
 *
 *  @addtogroup FlexCAN
 * @{
 */
...
#define FLEXCAN_IP_AR_RELEASE_MINOR_VERSION_C       9
#define FLEXCAN_IP_AR_RELEASE_REVISION_VERSION_C    0
#define FLEXCAN_IP_SW_MAJOR_VERSION_C               7
#define FLEXCAN_IP_SW_MINOR_VERSION_C               0
#define FLEXCAN_IP_SW_PATCH_VERSION_C               1
/*==================================================================================================
*                                     FILE VERSION CHECKS
==================================================================================================*/
...
/* Timeout max in milliseconds */
#define FLEXCAN_IP_TIMEOUT_MAX    (4294967UL)
/*==================================================================================================
*                                      LOCAL CONSTANTS
==================================================================================================*/
...
#if (FLEXCAN_IP_FEATURE_HAS_ENHANCED_RX_FIFO == STD_ON)
static void FlexCAN_IRQHandlerEnhancedRxFIFO(uint8 Instance, uint8 IntType);
...
static void FlexCAN_ProcessIRQHandlerError(uint8 Instance);

static void FlexCAN_ProcessIRQHandlerBusOff(uint8 Instance);

static uint32 FlexCAN_ErrorCallback(uint8 Instance, Flexcan_Ip_EventType ErrEvent, uint32 u32ErrStatus);

static void FlexCAN_ProcessIRQHandlerMsgBuffer (uint8 Instance, uint32 MbIdx);
...
#if (FLEXCAN_IP_DEV_ERROR_DETECT == STD_ON)
    DevAssert((boolean)(FlexCAN_IsEnhancedRxFifoAvailable(pBase)));
...
static Flexcan_Ip_StatusType FlexCAN_StartRxMessageEnhancedFifoData(uint8 Instance, Flexcan_Ip_MsgBuffType * Data)
{
    ...
    else
    {
        ...
        if (FLEXCAN_RXFIFO_USING_DMA == pState->transferType)
        {
            ...
            axTransferList[12].Value = (uint32)(0xFFFFFFB0U); /* enable for src address: after each minor loop, jump back to output of enhance fifo. (0xFFFFFFB0U) ~ (sint32)(-80) */
...
{
#if (FLEXCAN_IP_DEV_ERROR_DETECT == STD_ON)
    DevAssert((boolean)(u32TimeoutMs <= FLEXCAN_IP_TIMEOUT_MAX));
#endif
    Flexcan_Ip_StatusType eResult = FLEXCAN_STATUS_SUCCESS;
    ...
    uint32 Us2Ticks = OsIf_MicrosToTicks((u32TimeoutMs * 1000U), FLEXCAN_IP_SERVICE_TIMEOUT_TYPE);
    uint8 u8IntType = 0U;
    ...
    while (FLEXCAN_MB_RX_BUSY == pState->enhancedFifoOutput.state)
    {
        if (FLEXCAN_RXFIFO_USING_POLLING == pState->transferType)
        {
            for (u8IntType = FLEXCAN_IP_ENHANCED_RXFIFO_UNDERFLOW; \
                 u8IntType >= FLEXCAN_IP_ENHANCED_RXFIFO_FRAME_AVAILABLE; \
                 u8IntType--)
            {
                if (FlexCAN_GetEnhancedRxFIFOIntStatusFlag(pBase, u8IntType) != 0U)
                {
                    FlexCAN_IRQHandlerEnhancedRxFIFO(u8Instance, u8IntType);
                }
            }
        }
                                                  ...
                                                  )
{
    ...
    DevAssert((boolean)((TxInfo->data_length) <= FlexCAN_GetMbPayloadSize(pBase, MbIdx)));
#else
    DevAssert((boolean)((TxInfo->data_length) <= 8U));
#endif
 ...
 * for CAN interface.
 *
 * This is not a public API as it is called whenever receiving in individual MB occurs
 *END**************************************************************************/
static void FlexCAN_IRQHandlerRxMB(uint8 Instance, uint32 MbIdx)
{
    FLEXCAN_Type * pBase = Flexcan_Ip_apxBase[Instance];
 ...
 * This is not a public API as it is called whenever transmitting in individual MB occurs
 *END**************************************************************************/
static void FlexCAN_IRQHandlerTxMB(uint8 u8Instance, uint32 u32MbIdx)
{
    FLEXCAN_Type * pBase = Flexcan_Ip_apxBase[u8Instance];
    ...
    if ((FALSE == pState->mbs[u32MbIdx].isPolling) && ((FLEXCAN_MB_IDLE == pState->mbs[u32MbIdx].state)
       ))
    {
...
static inline void FlexCAN_ProcessIRQHandlerEnhancedRxFIFO(uint8 u8Instance)
{
    ...
    for (u8IntType = FLEXCAN_IP_ENHANCED_RXFIFO_UNDERFLOW; u8IntType >= FLEXCAN_IP_ENHANCED_RXFIFO_FRAME_AVAILABLE; u8IntType--)
    {
        /* Check spurious interrupt */
        if (((uint8)0U != FlexCAN_GetEnhancedRxFIFOIntStatusFlag(pBase, u8IntType)) && ((uint8)0U != FlexCAN_GetEnhancedRxFIFOIntStatusMask(pBase, u8IntType)))
        {
            /* Check current instance is initialized */
            if (NULL_PTR != pState)
            {
                if (FLEXCAN_RXFIFO_USING_INTERRUPTS == pState->transferType)
                {
            ...
            {
                FlexCAN_ClearEnhancedRxFifoIntStatusFlag(pBase, u8IntType);
            }
        }
    }
}
...
static void FlexCAN_CompleteRxMessageFifoData(uint8 Instance)
{
    ...
    else if (FLEXCAN_RXFIFO_USING_DMA == pState->transferType)
    {
        ...
        if (pState->mbs[FLEXCAN_IP_MB_HANDLE_RXFIFO].state != FLEXCAN_MB_DMA_ERROR)
        {
            ...
            /* Extract the IDHIT */
            FifoMessage->id_hit = (uint8)(((FifoMessage->cs & FLEXCAN_IP_CS_IDHIT_MASK) >> FLEXCAN_IP_CS_IDHIT_SHIFT) & (uint32)UINT8_MAX);
            /* Extract the Time Stamp */
            FifoMessage->time_stamp = (uint32)((FifoMessage->cs & FLEXCAN_IP_CS_TIME_STAMP_MASK) >> FLEXCAN_IP_CS_TIME_STAMP_SHIFT);
            /* Reverse the endianness */
...
/*FUNCTION**********************************************************************
 *
 * Function Name : FlexCAN_ProcessIRQHandlerError
 * Description   : Checking the error interrupts that are available and ready
...
static void FlexCAN_ProcessIRQHandlerError(uint8 Instance)
{
    ...
    if (((uint32)0U != (u32ErrStatus & ((uint32)FLEXCAN_ESR1_ERRINT_MASK))) && ((uint32)0U != (pBase->CTRL1 & ((uint32)FLEXCAN_CTRL1_ERRMSK_MASK))))
    {
        ...
        if (NULL_PTR != pState)
        {
            ...
            /* Invoke callback */
            u32ErrStatus = FlexCAN_ErrorCallback(Instance, FLEXCAN_EVENT_ERROR, u32ErrStatus);
        }
        else
        {
            pBase->ESR1 = FLEXCAN_ESR1_ERRINT_MASK;
...
#if (FLEXCAN_IP_FEATURE_HAS_FD == STD_ON)
    {
        ...
        if (((uint32)0U != (u32ErrStatus & ((uint32)FLEXCAN_ESR1_ERRINT_FAST_MASK))) && ((uint32)0U != (pBase->CTRL2 & ((uint32)FLEXCAN_CTRL2_ERRMSK_FAST_MASK))))
        {
            ...
            if (NULL_PTR != pState)
            {
                ...
                /* Invoke callback */
                (void)FlexCAN_ErrorCallback(Instance, FLEXCAN_EVENT_ERROR_FAST, u32ErrStatus);
            }
            else
            {
                pBase->ESR1 = FLEXCAN_ESR1_ERRINT_FAST_MASK;
...
/*FUNCTION**********************************************************************
 *
 * Function Name : FlexCAN_ProcessIRQHandlerBusOff
 * Description   : Checking the BusOff interrupts that are available and ready
 * for executing ErrorCallback function if this function is available.
 * This is not a public API as it is called from other driver functions.
 *
 *END**************************************************************************/
static void FlexCAN_ProcessIRQHandlerBusOff(uint8 Instance)
{
    ...
    if (((uint32)0U != (u32ErrStatus & ((uint32)FLEXCAN_ESR1_TWRNINT_MASK))) && (0U != (pBase->CTRL1 & ((uint32)FLEXCAN_CTRL1_TWRNMSK_MASK))))
    {
        ...
        if (NULL_PTR != pState)
        {
            ...
            /* Invoke callback */
            u32ErrStatus = FlexCAN_ErrorCallback(Instance, FLEXCAN_EVENT_TX_WARNING, u32ErrStatus);
        }
        else
        {
            pBase->ESR1 = FLEXCAN_ESR1_TWRNINT_MASK;
    ...
    if (((uint32)0U != (u32ErrStatus & ((uint32)FLEXCAN_ESR1_RWRNINT_MASK))) && (0U != (pBase->CTRL1 & ((uint32)FLEXCAN_CTRL1_RWRNMSK_MASK))))
    {
        ...
        if (NULL_PTR != pState)
        {
            ...
            /* Invoke callback */
            u32ErrStatus = FlexCAN_ErrorCallback(Instance, FLEXCAN_EVENT_RX_WARNING, u32ErrStatus);
        }
        else
        {
            pBase->ESR1 = FLEXCAN_ESR1_RWRNINT_MASK;
    ...
    if (((uint32)0U != (u32ErrStatus & ((uint32)FLEXCAN_ESR1_BOFFINT_MASK))) && ((uint32)0U != (pBase->CTRL1 & ((uint32)FLEXCAN_CTRL1_BOFFMSK_MASK))))
    {
        ...
        if (NULL_PTR != pState)
        {
            ...
            /* Invoke callback */
            (void)FlexCAN_ErrorCallback(Instance, FLEXCAN_EVENT_BUSOFF, u32ErrStatus);
        }
        else
        {
            pBase->ESR1 = FLEXCAN_ESR1_BOFFINT_MASK;
...
static uint32 FlexCAN_ErrorCallback(uint8 Instance, Flexcan_Ip_EventType ErrEvent, uint32 u32ErrStatus)
{
    ...
    {
        pState->error_callback(Instance, ErrEvent, u32ErrStatus, pState);
...
{
#if (FLEXCAN_IP_DEV_ERROR_DETECT == STD_ON)
    DevAssert((boolean)(Flexcan_Ip_u8Instance < FLEXCAN_IP_INSTANCE_COUNT));
    DevAssert((boolean)(Flexcan_Ip_pData != NULL_PTR));
#endif /* (FLEXCAN_IP_DEV_ERROR_DETECT == STD_ON) */
    Flexcan_Ip_StatusType eResult = FLEXCAN_STATUS_SUCCESS;
    ...
    if (FLEXCAN_STATUS_SUCCESS == eResult)
    {
        ...
        FlexCAN_ConfigTimestamp(Flexcan_Ip_u8Instance, (const Flexcan_Ip_TimeStampConfigType *)(&Flexcan_Ip_pData->time_stamp));
...
{
#if (FLEXCAN_IP_DEV_ERROR_DETECT == STD_ON)
    DevAssert((boolean)(instance < FLEXCAN_IP_INSTANCE_COUNT));
    DevAssert((boolean)(tx_info != NULL_PTR));
#endif
    ...
    /* Only call the send frame function when Listen-Only Mode is not enabled, or when the current frame is a Data frame and the system is in Restricted Mode to transmit only time reference messages */
...
#if (FLEXCAN_IP_DEV_ERROR_DETECT == STD_ON)
    DevAssert((boolean)(instance < FLEXCAN_IP_INSTANCE_COUNT));
    DevAssert((boolean)(rx_info != NULL_PTR));
    const Flexcan_Ip_StateType * pState = Flexcan_Ip_apxState[instance];
#endif
    Flexcan_Ip_StatusType eResult = FLEXCAN_STATUS_SUCCESS;
    Flexcan_Ip_MsbuffCodeStatusType Cs;
...
#if (FLEXCAN_IP_DEV_ERROR_DETECT == STD_ON)
    DevAssert((boolean)(instance < FLEXCAN_IP_INSTANCE_COUNT));
    DevAssert((boolean)((id_filter_table != NULL_PTR) || (FLEXCAN_RX_FIFO_ID_FORMAT_D == id_format)));
#endif
    Flexcan_Ip_StatusType Result = FLEXCAN_STATUS_SUCCESS;
...
#if (FLEXCAN_IP_DEV_ERROR_DETECT == STD_ON)
    if ((FALSE == FlexCAN_IsFreezeMode(pBase)) && (FLEXCAN_STATUS_SUCCESS == Result))
    {
...
{
#if (FLEXCAN_IP_DEV_ERROR_DETECT == STD_ON)
    DevAssert((boolean)(instance < FLEXCAN_IP_FEATURE_ENHANCED_RX_FIFO_NUM));
    DevAssert((boolean)(id_filter_table != NULL_PTR));
#endif
    Flexcan_Ip_StatusType Result = FLEXCAN_STATUS_SUCCESS;
    Flexcan_Ip_StatusType Status = FLEXCAN_STATUS_SUCCESS;
    FLEXCAN_Type * pBase = Flexcan_Ip_apxBase[instance];
                                                       ...
                                                       )
{
    ...
    DevAssert((boolean)(0U == (pBase->CTRL2 & FLEXCAN_CTRL2_RRS_MASK)));
#if (FLEXCAN_IP_FEATURE_HAS_FD == STD_ON)
    /* Check if the Payload Size is smaller than the payload configured */
    DevAssert((boolean)((tx_info->data_length) <= FlexCAN_GetMbPayloadSize(pBase, mb_idx)));
#else
    DevAssert((boolean)((tx_info->data_length) <= 8U));
#endif
...
Flexcan_Ip_StatusType FlexCAN_Ip_GetTransferStatus(uint8 instance, uint8 mb_idx)
{
    ...
    DevAssert((boolean)(instance < FLEXCAN_IP_INSTANCE_COUNT));
    #if (FLEXCAN_IP_FEATURE_HAS_ENHANCED_RX_FIFO == STD_ON)
    DevAssert((boolean)((mb_idx < (uint8)FLEXCAN_IP_FEATURE_MAX_MB_NUM) || (FLEXCAN_IP_MB_ENHANCED_RXFIFO == mb_idx)));
    #else
    DevAssert((boolean)(mb_idx < (uint8)FLEXCAN_IP_FEATURE_MAX_MB_NUM));
    #endif
#endif
    const Flexcan_Ip_StateType * pState = Flexcan_Ip_apxState[instance];
 ...
 *END**************************************************************************/
/* implements FlexCAN_IRQHandler_Activity */
...
)
{
#if (FLEXCAN_IP_DEV_ERROR_DETECT == STD_ON)
    DevAssert((boolean)(Instance < FLEXCAN_IP_INSTANCE_COUNT));
    DevAssert((boolean)(EndMbIdx < (uint8)FLEXCAN_IP_FEATURE_MAX_MB_NUM));
#endif
    ...
    if (TRUE == bInterruptValid)
    {
        ...
        if (NULL_PTR != pState)
        {
            FlexCAN_ProcessIRQHandlerMsgBuffer(Instance, MbIdx);
        }
        else
        {
            FlexCAN_ClearMsgBuffIntStatusFlag(pBase, MbIdx);
 ...
 *
 *END**************************************************************************/
/* implements  FlexCAN_Busoff_Error_IRQHandler_Activity */
void FlexCAN_Busoff_Error_IRQHandler(uint8 Instance)
{
    ...
    FlexCAN_ProcessIRQHandlerError(Instance);

    FlexCAN_ProcessIRQHandlerBusOff(Instance);

}
#endif /* (FLEXCAN_IP_FEATURE_BUSOFF_ERROR_INTERRUPT_UNIFIED == STD_ON) */
                                             ...
                                             )
{
    ...
    DevAssert((boolean)(instance < FLEXCAN_IP_INSTANCE_COUNT));
    DevAssert((boolean)(tx_info != NULL_PTR));
    DevAssert((boolean)(timeout_ms <= FLEXCAN_IP_TIMEOUT_MAX));
#endif
    Flexcan_Ip_StatusType Result = FLEXCAN_STATUS_ERROR;
    FLEXCAN_Type * pBase = Flexcan_Ip_apxBase[instance];
    Flexcan_Ip_StateType * pState = Flexcan_Ip_apxState[instance];
    ...
    if (FLEXCAN_STATUS_SUCCESS == Result)
    {
        Us2Ticks = OsIf_MicrosToTicks((timeout_ms * 1000U), FLEXCAN_IP_SERVICE_TIMEOUT_TYPE);
        TimeStart = OsIf_GetCounter(FLEXCAN_IP_SERVICE_TIMEOUT_TYPE);
...
Flexcan_Ip_StatusType FlexCAN_Ip_Deinit_Privileged(uint8 instance)
{
    ...
    if (FLEXCAN_STATUS_SUCCESS == Result)
    {
        ...
        /* wait for disable */
        (void)FlexCAN_Disable(pBase);
        /* Clear state pointer that is checked by FlexCAN_Ip_Init */
...
void FlexCAN_Ip_MainFunctionRead(uint8 instance, uint8 mb_idx)
{
    ...
    if ((uint8)FLEXCAN_IP_MB_ENHANCED_RXFIFO == mb_idx)
    {
        if (TRUE == FlexCAN_IsEnhancedRxFifoEnabled(pBase))
        {
            if (FlexCAN_GetEnhancedRxFIFOIntStatusFlag(pBase, FLEXCAN_IP_ENHANCED_RXFIFO_FRAME_AVAILABLE) != 0U)
            {
...
boolean FlexCAN_Ip_GetStopMode(uint8 instance)
{
    const FLEXCAN_Type * pBase = Flexcan_Ip_apxBase[instance];
    uint32 Value;

#if ((STD_ON == FLEXCAN_IP_ENABLE_USER_MODE_SUPPORT) && (STD_OFF == FLEXCAN_IP_SET_USER_ACCESS_ALLOWED_AVAILABLE))
    Value = OsIf_Trusted_Call_Return1param(FlexCAN_GetStopMode, pBase);
#else
    Value = FlexCAN_GetStopMode(pBase);
#endif
...
boolean FlexCAN_Ip_GetStartMode(uint8 instance)
{
    const FLEXCAN_Type * pBase = Flexcan_Ip_apxBase[instance];
    uint32 Value;

#if ((STD_ON == FLEXCAN_IP_ENABLE_USER_MODE_SUPPORT) && (STD_OFF == FLEXCAN_IP_SET_USER_ACCESS_ALLOWED_AVAILABLE))
    Value = OsIf_Trusted_Call_Return1param(FlexCAN_GetStartMode, pBase);
#else
    Value = FlexCAN_GetStartMode(pBase);
#endif
...
#if (FLEXCAN_IP_DEV_ERROR_DETECT == STD_ON)
    DevAssert((boolean)(instance < FLEXCAN_IP_INSTANCE_COUNT));
    DevAssert((boolean)(bitrate != NULL_PTR));
#endif
    Flexcan_Ip_StatusType Result = FLEXCAN_STATUS_SUCCESS;
    Flexcan_Ip_StatusType Status = FLEXCAN_STATUS_SUCCESS;
    FLEXCAN_Type * pBase = Flexcan_Ip_apxBase[instance];
...
{
#if (FLEXCAN_IP_DEV_ERROR_DETECT == STD_ON)
    DevAssert((boolean)(u8Instance < FLEXCAN_IP_INSTANCE_COUNT));
#endif
    FLEXCAN_Type * pBase = Flexcan_Ip_apxBase[u8Instance];
...
#if (FLEXCAN_IP_DEV_ERROR_DETECT == STD_ON)
    /* Check if controller is in freeze mode or not */
    if ((FALSE == FdEnable) || ((FALSE == FlexCAN_IsFreezeMode(pBase)) && (FLEXCAN_STATUS_SUCCESS == Result)))
    {
        Result = FLEXCAN_STATUS_ERROR;
    }
#endif
...
boolean FlexCAN_Ip_GetBuffStatusFlag(uint8 instance, uint8 msgBuffIdx)
{
    ...
    {
        ReturnResult = ((1U == FlexCAN_GetEnhancedRxFIFOIntStatusFlag(pBase, FLEXCAN_IP_ENHANCED_RXFIFO_OVERFLOW)) ? TRUE : FALSE);
    }
    else
#endif
    {
...
Flexcan_Ip_StatusType FlexCAN_Ip_ConfigTimeStamp_Privileged(uint8 instance, const Flexcan_Ip_TimeStampConfigType * time_stamp)
{
    ...
    if (FLEXCAN_STATUS_SUCCESS == Result)
    {
        FlexCAN_ConfigTimestamp(instance, time_stamp);
    }
 ...
 *
 *END**************************************************************************/
/* implements  FlexCAN_MemError_IRQHandler_Activity */
...
Flexcan_Ip_StatusType FlexCAN_Ip_SetMemErrorDetection(uint8 Instance, boolean IsEnable)
{
    ...
    if (NULL_PTR != pState)
    {
        ...
        if (TRUE == IsEnable)
        {
            /* check the response when non-correctable error is detected in a memory read performed by FlexCAN internal processes */
                                                      ...
                                                      boolean IsEnable)
{
#if (FLEXCAN_IP_DEV_ERROR_DETECT == STD_ON)
    DevAssert((boolean)(Instance < FLEXCAN_IP_INSTANCE_COUNT));
    DevAssert((boolean)(MemErrorInjection != NULL_PTR));
#endif
    FLEXCAN_Type * pBase = Flexcan_Ip_apxBase[Instance];
    ...
    if (bErrCfgEnabled && bErrDetEnabled)
    {
        ...
        else
        {
            ...
            /* clear specific error injection */
            switch (MemErrorInjection->eErrorInjection)
            {
                case FLEXCAN_HOST_ACCESS_ERROR_INJECTION:
                    FlexCAN_ClearMemErrorInjection(pBase, FLEXCAN_INJECT_HOST_ACCESS_ERROR);
                    break;
                case FLEXCAN_FLEXCAN_ACCESS_ERROR_INJECTION:
                    FlexCAN_ClearMemErrorInjection(pBase, FLEXCAN_INJECT_FLEXCAN_ACCESS_ERROR);
                    break;
                case FLEXCAN_EXTENDED_FLEXCAN_ACCESS_ERROR_INJECTION:
                    FlexCAN_ClearMemErrorInjection(pBase, FLEXCAN_INJECT_EXTENDED_FLEXCAN_ACCESS_ERROR);
                    break;
                default:
                    /* nothing */
                    #if (FLEXCAN_IP_DEV_ERROR_DETECT == STD_ON)
    ...
    if (TRUE == bErrDetEnabled)
    {
        ...
        if ((uint32)0U != u32ErrStatus)
        {
        ...
        #endif
            {
                ...
                pBase->ERRSR = (FLEXCAN_ERRSR_ERROR_FLAG_MASK | FLEXCAN_ERRSR_OVERRUN_FLAG_MASK);
                /* Invoke callback */
                (void)FlexCAN_ErrorCallback(Instance, FLEXCAN_EVENT_MEM_ERROR_DETECT, u32ErrStatus);
...
#if (FLEXCAN_IP_DEV_ERROR_DETECT == STD_ON)
    DevAssert((boolean)(Instance < FLEXCAN_IP_INSTANCE_COUNT));
    DevAssert((boolean)(ErrorReport != NULL_PTR));
#endif
    FLEXCAN_Type * pBase = Flexcan_Ip_apxBase[Instance];
    Flexcan_Ip_StateType * pState = Flexcan_Ip_apxState[Instance];
    Flexcan_Ip_StatusType RetVal = FLEXCAN_STATUS_ERROR;
