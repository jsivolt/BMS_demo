...
#define ADC_SAR_IP_AR_RELEASE_MINOR_VERSION_C       9
#define ADC_SAR_IP_AR_RELEASE_REVISION_VERSION_C    0
#define ADC_SAR_IP_SW_MAJOR_VERSION_C               7
...
#if ((STD_ON == ADC_SAR_IP_SELFTEST_ENABLED) \
     || (STD_ON == ADC_SAR_IP_CALIBRATION_AVAILABLE) \
    )
static inline Adc_Sar_Ip_StatusType Adc_Sar_CheckFlagWithTimeout(volatile const uint32 * registerAddress,
                                                                 uint32 registerMask,
                                                                 uint32 testValue);
#endif /*  ADC_SAR_IP_SELFTEST_ENABLED ||  */
#if ( \
...
static inline void Adc_Sar_SetupCalibration(volatile uint32 * CALBISTREGAddr,
...
static inline void Adc_Sar_EnableHighSpeed(ADC_Type * const Base, boolean Enable);
...
static inline Adc_Sar_Ip_StatusType Adc_Sar_CheckTimeoutStatus(const uint32 ActualTimeout,
                                                               const uint32 ExpectTimeout)
{
    Adc_Sar_Ip_StatusType RelValue = ADC_SAR_IP_STATUS_SUCCESS;
    /* Check if the timeout is not expected */
    if (ActualTimeout >= ExpectTimeout)
    {
        RelValue = ADC_SAR_IP_STATUS_TIMEOUT;
    }
                                                   ...
                                                   const Adc_Sar_Ip_SelfTestThresholdType * SelfTestThresholdConfig)
{
...

    {
        AdcBasePtr = Adc_Sar_Ip_apxAdcBase[Instance];
#ifdef ADC_STAW0R_AWDE
        STAW0RAddr = (volatile uint32 *)&(AdcBasePtr->STAW0R);
#endif /* ADC_STAW0R_AWDE */
#ifdef ADC_STAW1R_AWDE
        STAW1RAddr = (volatile uint32 *)&(AdcBasePtr->STAW1R);
#endif /* ADC_STAW1R_AWDE */
#ifdef ADC_STAW1AR_AWDE
        STAW1ARAddr = (volatile uint32 *)&(AdcBasePtr->STAW1AR);
        STAW1BRAddr = (volatile uint32 *)&(AdcBasePtr->STAW1BR);
#endif /* ADC_STAW1AR_AWDE */
#ifdef ADC_STAW2R_AWDE
        STAW2RAddr = (volatile uint32 *)&(AdcBasePtr->STAW2R);
#endif /* ADC_STAW2R_AWDE */
#ifdef ADC_STAW4R_AWDE
        STAW4RAddr = (volatile uint32 *)&(AdcBasePtr->STAW4R);
        STAW5RAddr = (volatile uint32 *)&(AdcBasePtr->STAW5R);
#endif /* ADC_STAW4R_AWDE */
    }
...
static inline Adc_Sar_Ip_StatusType Adc_Sar_CheckSelfTestProgress(const uint32 Instance)
{
    ...
    */
    ...
    {
        /* Wait until selftest has started*/
        Status = Adc_Sar_CheckFlagWithTimeout(MSRAddr, ADC_MSR_SELF_TEST_S_MASK, ADC_MSR_SELF_TEST_S_MASK);
        if (Status == ADC_SAR_IP_STATUS_SUCCESS)
        {
            /* Wait until Alg S has finished*/
            Status = Adc_Sar_CheckFlagWithTimeout(MSRAddr, ADC_MSR_SELF_TEST_S_MASK, 0U);
        }
                                     ...
                                     const uint32 Length)
{
    ...
    DevAssert((boolean)((ResultsRaw != NULL_PTR) || (ResultsStruct != NULL_PTR)));
...
/*FUNCTION*********************************************************************
 *
 * Function Name : Adc_Sar_CheckFlagWithTimeout
 * Description   : This function perform wait and check for a mask with timeout.
 *
 *END*************************************************************************/
static inline Adc_Sar_Ip_StatusType Adc_Sar_CheckFlagWithTimeout(volatile const uint32 * registerAddress,
                                                                 uint32 registerMask,
                                                                 uint32 testValue)
{
    ...
    while ((((*registerAddress) & registerMask) != testValue)  && (ElapsedTicks < TimeoutTicks))
    {
        ElapsedTicks += OsIf_GetElapsed(&CurrentTicks, ADC_SAR_IP_TIMEOUT_TYPE);
    }
    if (ElapsedTicks >= TimeoutTicks)
    {
        /* Timeout occurred while waiting for condition to set */
...
static inline void Adc_CheckAndCallAllChannelNotification(const uint32 Instance)
{
...

    {
        AdcBasePtr = Adc_Sar_Ip_apxAdcBase[Instance];
#if (STD_ON == ADC_SAR_IP_EOC_ENABLED)
        IMRAddr = (volatile uint32 *)&(AdcBasePtr->IMR);
        ISRAddr = (volatile uint32 *)&(AdcBasePtr->ISR);
#endif /* (STD_ON == ADC_SAR_IP_EOC_ENABLED) */
#if (STD_ON == ADC_SAR_IP_WDG_ENABLED)
        WTISRAddr = (volatile uint32 *)&(AdcBasePtr->WTISR);
#endif /* (STD_ON == ADC_SAR_IP_WDG_ENABLED) */
    }
#if (STD_ON == ADC_SAR_IP_EOC_ENABLED)
...
void Adc_Sar_Ip_IRQHandler(const uint32 Instance)
{
    ...
    volatile uint32 * WTISRAddr = NULL_PTR;
    uint32 Wtimr = 0u;
    uint32 EnabledWdgThresholdFlags = 0u;
    uint8 GroupIdx = 0u;
    uint32 Aworr = 0u;
    uint32 Cwenr = 0u;
    uint32 EnabledWdgChannelFlags = 0u;
#endif /* (STD_ON == ADC_SAR_IP_WDG_ENABLED) */

#if (ADC_SAR_IP_DEV_ERROR_DETECT == STD_ON)
    DevAssert((boolean)(Instance < ADC_SAR_IP_INSTANCE_COUNT));
#endif /* (ADC_SAR_IP_DEV_ERROR_DETECT == STD_ON) */
    ...
    /* CPR_RTD_00012 */
        ...
        if ((((*ISRAddr) & ADC_ISR_JECH_MASK) != 0U) && ((Imr & ADC_IMR_MSKJECH_MASK)!= 0U))
        {
            /* Clear JECH interrupt flag */
            *ISRAddr = ADC_ISR_JECH_MASK;
        }
#endif /* (STD_ON == ADC_SAR_IP_JECH_ENABLED) */
#if (STD_ON == ADC_SAR_IP_EOC_ENABLED)
        /* Clear EOC or JEOC interrupt flag */
        if ((*ISRAddr & ADC_ISR_EOC_MASK) && (Imr & ADC_IMR_MSKEOC_MASK))
        {
            *ISRAddr = ADC_ISR_EOC_MASK;
        }
        if ((*ISRAddr & ADC_ISR_JEOC_MASK) && (Imr & ADC_IMR_MSKJEOC_MASK))
        {
            *ISRAddr = ADC_ISR_JEOC_MASK;
        }
#endif /* (STD_ON == ADC_SAR_IP_EOC_ENABLED) */
#if (STD_ON == ADC_SAR_IP_WDG_ENABLED)
        /* Clear WTISR flags for enabled WDG threshold interrupts */
        EnabledWdgThresholdFlags = *WTISRAddr & Wtimr;

        if (EnabledWdgThresholdFlags != 0U)
        {
            *WTISRAddr = EnabledWdgThresholdFlags;
        }
        for (GroupIdx = 0U; GroupIdx < Adc_Sar_Ip_au8AdcGroupCount[Instance]; GroupIdx++)
        {
            {
                Aworr = AWORR(AdcBasePtr, GroupIdx);
                Cwenr = CWENR(AdcBasePtr, GroupIdx);
                EnabledWdgChannelFlags = Aworr & Cwenr;
                if (EnabledWdgChannelFlags != 0U)
                {
                    AWORR(AdcBasePtr, GroupIdx) = EnabledWdgChannelFlags;
                }
            }
        }
#endif /* (STD_ON == ADC_SAR_IP_WDG_ENABLED) */
 ...
 * Description   : This function configures ADC clock for calibration.
 *
 *END**************************************************************************/

static inline Adc_Sar_Ip_StatusType Adc_Sar_CalibrationClkSelect(const uint32 u32Instance, volatile uint32 * MCRAddr, uint32 * McrSavedValue)
{
    uint32 Mcr;
    Adc_Sar_Ip_StatusType Status = ADC_SAR_IP_STATUS_SUCCESS;
...
/*FUNCTION**********************************************************************
 *
 * Function Name : Adc_Sar_CalibrationClkRestore
 * Description   : This function restore ADC clock after calibration.
 *
 *END**************************************************************************/
static inline Adc_Sar_Ip_StatusType Adc_Sar_CalibrationClkRestore(const uint32 u32Instance, volatile uint32 * MCRAddr, uint32 McrSavedValue)
{
    uint32 Mcr;
    Adc_Sar_Ip_StatusType Status = ADC_SAR_IP_STATUS_SUCCESS;

#if ADC_SAR_IP_HAS_ADCLKSEL
        /* Restore state of ADCLKSEL  */
    uint32 AdcClkSel = McrSavedValue & ADC_MCR_ADCLKSEL_MASK;
#endif /* ADC_SAR_IP_HAS_ADCLKSEL */

    /* Restore state of ADCKO */
    uint32 AdcAutoClkOff = McrSavedValue & ADC_MSR_ACKO_MASK;
...
#if (ADC_SAR_IP_ASYNC_CALIBARTION_ENABLED == STD_ON)
    Adc_Sar_Ip_axAdcSarState[u32Instance].CalibrationOnGoing = FALSE;
#endif /*(ADC_SAR_IP_ASYNC_CALIBARTION_ENABLED == STD_ON) */
    return Status;
}
...
/*FUNCTION**********************************************************************
 *
 * Function Name : Adc_Sar_SetupCalibration
 * Description   : This function setup calibration parameters.
 *
 *END**************************************************************************/
#if (!ADC_SAR_IP_CALIBRATION_USES_MCR || ADC_SAR_IP_APPLICATION_EXTENSION_AVAILABLE)
#if defined(ADC_CAL2_ENX_MASK)
static inline void Adc_Sar_SetupCalibration(volatile uint32 * CALBISTREGAddr, volatile uint32 * CAL2Addr)
#else
static inline void Adc_Sar_SetupCalibration(volatile uint32 * CALBISTREGAddr)
#endif /* defined(ADC_CAL2_ENX_MASK) */
{
...
#if (ADC_SAR_IP_DEV_ERROR_DETECT == STD_ON)
    DevAssert((boolean)(u32Instance < ADC_SAR_IP_INSTANCE_COUNT));
    DevAssert((boolean)(pConfig != NULL_PTR));
#if ADC_SAR_IP_HAS_CTU
                            ...
                            const Adc_Sar_Ip_ConvChainType pChainType)
{
#if (ADC_SAR_IP_DEV_ERROR_DETECT == STD_ON)
    DevAssert((boolean)(u32Instance < ADC_SAR_IP_INSTANCE_COUNT));
    DevAssert((boolean)(pChansIdxMask != NULL_PTR));
#if ADC_SAR_IP_BAD_ACCESS_PROT_CHANNEL
    uint8 i;
    ...
    {
        DevAssert((boolean)((pChansIdxMask->ChanMaskArr[i] & (~Adc_Sar_Ip_au32AdcChanBitmap[u32Instance][i])) == 0u));
    }
#endif /* ADC_SAR_IP_BAD_ACCESS_PROT_CHANNEL */
#endif /* (ADC_SAR_IP_DEV_ERROR_DETECT == STD_ON) */
...
#if (ADC_SAR_IP_DEV_ERROR_DETECT == STD_ON)
    DevAssert((boolean)(u32Instance < ADC_SAR_IP_INSTANCE_COUNT));
    DevAssert((boolean)(u32ChnIdx < ADC_SAR_IP_CDR_COUNT));
#if FEATURE_ADC_SAF8544_CHANNEL_WORKAROUND
    DevAssert((boolean)(!((0U == u32ChnIdx) && ((ADC_SAR_IP_CONV_CHAIN_INJECTED == pChainType) || (ADC_SAR_IP_CONV_CHAIN_CTU == pChainType)))));
#endif
#if ADC_SAR_IP_BAD_ACCESS_PROT_CHANNEL
    DevAssert((boolean)ADC_SAR_IP_INST_HAS_CHANn(u32Instance, u32ChnIdx));
#endif /* ADC_SAR_IP_BAD_ACCESS_PROT_CHANNEL */
#endif /* (ADC_SAR_IP_DEV_ERROR_DETECT == STD_ON) */
                               ...
                               const uint32 u32ChnIdx)
{
    ...
    DevAssert((boolean)(u32ChnIdx < ADC_SAR_IP_CDR_COUNT));
#if FEATURE_ADC_SAF8544_CHANNEL_WORKAROUND
    DevAssert((boolean)((0U == u32ChnIdx) && (ADC_SAR_IP_CONV_CHAIN_INJECTED == pChainType)));
#endif
#if ADC_SAR_IP_BAD_ACCESS_PROT_CHANNEL
    DevAssert((boolean)ADC_SAR_IP_INST_HAS_CHANn(u32Instance, u32ChnIdx));
#endif /* ADC_SAR_IP_BAD_ACCESS_PROT_CHANNEL */
                              ...
                              const Adc_Sar_Ip_Resolution eResolution)
{
...
#endif /*(ADC_SAR_IP_ASYNC_CALIBARTION_ENABLED == STD_ON) */
    {
        ...
        if ((Msr & ADC_MSR_ADCSTATUS_MASK) == ADC_MSR_ADCSTATUS(ADC_SAR_IP_MSR_ADCSTATUS_IDLE))
        {
...
#if (ADC_SAR_IP_DEV_ERROR_DETECT == STD_ON)
    DevAssert((boolean)(u32Instance < ADC_SAR_IP_INSTANCE_COUNT));
    DevAssert((boolean)((u32Mask & (~ADC_SAR_IP_NOTIF_FLAG_ALL)) == 0UL));
#endif /* (ADC_SAR_IP_DEV_ERROR_DETECT == STD_ON) */
...
Adc_Sar_Ip_StatusType Adc_Sar_Ip_SelfTest(const uint32 u32Instance)
{
...
#endif /* ADC_SAR_IP_HAS_SELFTEST_USE_CH32 */
#if ADC_SAR_IP_HAS_SELFTEST_STCR1
            STCR1Addr = (volatile uint32 *)&(AdcBasePtr->STCR1);
#endif /* ADC_SAR_IP_HAS_SELFTEST_STCR1 */
            STCR2Addr = (volatile uint32 *)&(AdcBasePtr->STCR2);
#if ADC_SAR_IP_HAS_SELFTEST_STCR3
            STCR3Addr = (volatile uint32 *)&(AdcBasePtr->STCR3);
#endif /* ADC_SAR_IP_HAS_SELFTEST_STCR3 */
        }
...
#endif /* ADC_SAR_IP_SELFTEST_FULL_CLK */
                    {
                        Status = Adc_Sar_CheckTimeoutStatus(ElapsedTicks, TimeoutTicks);
                    }
#if ADC_SAR_IP_SELFTEST_FULL_CLK
                }
            }
    ...
    {
        Status = ADC_SAR_IP_STATUS_CAL_ONGOING;
    }
#endif /*(ADC_SAR_IP_ASYNC_CALIBARTION_ENABLED == STD_ON) */
    return Status;
}
                              ...
                              Adc_Sar_Ip_ChanResultType * const pResult)
{
    ...
    DevAssert((boolean)(u32Instance < ADC_SAR_IP_INSTANCE_COUNT));
    DevAssert((boolean)(u32ChnIdx < ADC_SAR_IP_CDR_COUNT));
    DevAssert((boolean)(pResult != NULL_PTR));
#if ADC_SAR_IP_BAD_ACCESS_PROT_CHANNEL
    DevAssert((boolean)ADC_SAR_IP_INST_HAS_CHANn(u32Instance, u32ChnIdx));
#endif /* ADC_SAR_IP_BAD_ACCESS_PROT_CHANNEL */
#endif /* (ADC_SAR_IP_DEV_ERROR_DETECT == STD_ON) */
...
Adc_Sar_Ip_StatusType Adc_Sar_Ip_DoCalibration(const uint32 u32Instance)
{
    ...
    static uint32 McrSavedValue = 0u;
    volatile uint32 * MCRAddr = NULL_PTR;
#if (!ADC_SAR_IP_CALIBRATION_USES_MCR || ADC_SAR_IP_APPLICATION_EXTENSION_AVAILABLE)
    volatile uint32 * CALBISTREGAddr = NULL_PTR;
#if defined(ADC_CAL2_ENX_MASK)
...

    {
        ...
        CALBISTREGAddr = (volatile uint32 *)&(AdcBasePtr->CALBISTREG);
#if defined(ADC_CAL2_ENX_MASK)
        CAL2Addr = (volatile uint32 *)&(AdcBasePtr->CAL2);
#endif /* defined(ADC_CAL2_ENX_MASK) */
#endif /* !ADC_SAR_IP_CALIBRATION_USES_MCR */
    }
...
#endif /*(ADC_SAR_IP_ASYNC_CALIBARTION_ENABLED == STD_ON) */
                    Status = Adc_Sar_CalibrationClkSelect(u32Instance, MCRAddr, &McrSavedValue);
                    if (ADC_SAR_IP_STATUS_SUCCESS == Status)
                    {
                #if defined(ADC_CAL2_ENX_MASK)
                        Adc_Sar_SetupCalibration(CALBISTREGAddr, CAL2Addr);
                #else
                        Adc_Sar_SetupCalibration(CALBISTREGAddr);
                #endif /* defined(ADC_CAL2_ENX_MASK) */
                    }
#if (ADC_SAR_IP_ASYNC_CALIBARTION_ENABLED == STD_ON)
                    Status = ADC_SAR_IP_STATUS_CAL_ONGOING;
...
#endif /*(ADC_SAR_IP_ASYNC_CALIBARTION_ENABLED == STD_ON) */
                {
                    ...
                    CalStatus = Adc_Sar_CheckFlagWithTimeout(CALBISTREGAddr, ADC_SAR_IP_CALBISTREG_C_T_BUSY_MASK, 0U);
                    if (ADC_SAR_IP_STATUS_SUCCESS == CalStatus)
                    {
                        if (((*CALBISTREGAddr) & ADC_SAR_IP_CALBISTREG_TEST_FAIL_MASK) != 0U)
                        {
                                           ...
                                           const uint32 u32Mask)
{
    ...
    DevAssert((boolean)(u32Instance < ADC_SAR_IP_INSTANCE_COUNT));
    DevAssert((boolean)(u32ChnIdx < ADC_SAR_IP_CDR_COUNT));
    DevAssert((boolean)((u32Mask & ~(ADC_SAR_IP_CHAN_NOTIF_FLAG_ALL)) == 0UL));
#if ADC_SAR_IP_BAD_ACCESS_PROT_CHANNEL
    DevAssert((boolean)ADC_SAR_IP_INST_HAS_CHANn(u32Instance, u32ChnIdx));
#endif /* ADC_SAR_IP_BAD_ACCESS_PROT_CHANNEL */
#endif /* (ADC_SAR_IP_DEV_ERROR_DETECT == STD_ON) */
                                            ...
                                            const boolean bAllowRestart)
{
    ...
    {
        ReturnStatus = ADC_SAR_IP_STATUS_CAL_ONGOING;
    }
#endif /*(ADC_SAR_IP_ASYNC_CALIBARTION_ENABLED == STD_ON) */
    return ReturnStatus;
}
...
#if (ADC_SAR_IP_DEV_ERROR_DETECT == STD_ON)
    DevAssert((boolean)(u32Instance < ADC_SAR_IP_INSTANCE_COUNT));
    DevAssert((boolean)(pThresholdValues != NULL_PTR));
    DevAssert((boolean)(u8RegisterIdx < ADC_SAR_IP_THRHLR_COUNT));
#endif /* (ADC_SAR_IP_DEV_ERROR_DETECT == STD_ON) */
                                  ...
                                  const Adc_Sar_Ip_ConvModeType eConvMode)
{
...
#endif /*(ADC_SAR_IP_ASYNC_CALIBARTION_ENABLED == STD_ON) */
    {
        ...
        /* The selected ADC should be in IDLE state */
        if (((*MSRAddr) & ADC_MSR_ADCSTATUS_MASK) == ADC_MSR_ADCSTATUS(ADC_SAR_IP_MSR_ADCSTATUS_IDLE))
        {
            /* Edit MODE */
                                            ...
                                            const Adc_Sar_Ip_CtuModeType eCtuMode)
{
    ...
    DevAssert((boolean)(u32Instance < ADC_SAR_IP_INSTANCE_COUNT));
#if ADC_SAR_IP_BAD_ACCESS_PROT_FEATURE
    DevAssert((boolean)(!((FALSE == ADC_SAR_IP_INST_HAS_CTU_TRIGGER_MODE(u32Instance)) && (ADC_SAR_IP_CTU_MODE_TRIGGER == eCtuMode))));
#endif /*ADC_SAR_IP_BAD_ACCESS_PROT_FEATURE*/
#endif /* (ADC_SAR_IP_DEV_ERROR_DETECT == STD_ON) */
#if (ADC_SAR_IP_ASYNC_CALIBARTION_ENABLED == STD_ON)
    /* calibration must not be interrupted */
...
#if (ADC_SAR_IP_DEV_ERROR_DETECT == STD_ON)
    DevAssert((boolean)(u32Instance < ADC_SAR_IP_INSTANCE_COUNT));
    DevAssert((boolean)(u32ChannelIndex < ADC_SAR_IP_CDR_COUNT));
#if ADC_SAR_IP_BAD_ACCESS_PROT_CHANNEL
    DevAssert((boolean)ADC_SAR_IP_INST_HAS_CHANn(u32Instance, u32ChannelIndex));
#endif /* ADC_SAR_IP_BAD_ACCESS_PROT_CHANNEL */
#endif /* (ADC_SAR_IP_DEV_ERROR_DETECT == STD_ON) */
